...
function createPracticeService(options) {
  ...
  function initSchema() {
    // ENG-003: practice schema is created and recorded by migrations/002_practice.sql.
    // Every statement is idempotent, so this is a safe no-op (only records the baseline)
    // on the existing production database.
    runMigrations(db, { dir: MIGRATIONS_DIR, ids: ["002_practice.sql"] });
    // Idempotent legacy column backfill for databases created before the migration
    // baseline. No-op on fresh databases and on current production.
